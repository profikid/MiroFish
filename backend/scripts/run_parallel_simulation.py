...
Run Twitter and Reddit simulations concurrently, reading the same config file
...
# ============================================================
# Resolve Windows encoding issue: set UTF-8 encoding before any import
...
if sys.platform == 'win32':
    ...
    # This affects all open() calls that do not specify encoding
    os.environ.setdefault('PYTHONUTF8', '1')
    os.environ.setdefault('PYTHONIOENCODING', 'utf-8')
    
    # Reconfigure stdout streams to UTF-8 (fixes Chinese mojibake in the console)
    if hasattr(sys.stdout, 'reconfigure'):
        sys.stdout.reconfigure(encoding='utf-8', errors='replace')
    if hasattr(sys.stderr, 'reconfigure'):
        sys.stderr.reconfigure(encoding='utf-8', errors='replace')
    
    # Force the default encoding (affects open() default encoding)
    # Note: this must be set at Python startup; setting at runtime may not take effect
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        Wrap the open() function to default to UTF-8 for text mode
        This fixes issues where third-party libs (e.g. OASIS) read files without specifying encoding
        """
        # Only set the default encoding for text mode (not binary) and when no encoding is specified
...
# The script is fixed under the backend/scripts/ directory
...
def disable_oasis_logging():
    ...
    OASIS logs are too verbose (records every agent's observations and actions); we use our own action_logger
    """
...
class ParallelIPCHandler:
    ...
    Manage environments of both platforms and handle Interview commands
    """
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with results, or a dictionary containing an error
        """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/not specified: interview both platforms at once and return merged results
        ...
        # No platform specified: interview both platforms at once
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform (can be overridden per interview item)
                - "twitter": only interview the Twitter platform
                - "reddit": only interview the Reddit platform
                - None/not specified: each Agent is interviewed on both platforms simultaneously
        """
        # Group by platform
        twitter_interviews = []
        reddit_interviews = []
        ...
        for interview in interviews:
            ...
            if item_platform == "twitter":
                ...
                # Platform not specified: interview on both platforms
            ...
            try:
                ...
                for interview in twitter_interviews:
                    ...
                    try:
                        ...
                        print(f"  Warning: unable to fetch Twitter Agent {agent_id}: {e}")
            ...
            try:
                ...
                for interview in reddit_interviews:
                    ...
                    try:
                        ...
                        print(f"  Warning: unable to fetch Reddit Agent {agent_id}: {e}")
...
# Action type mapping table (DB name -> standard name)
ACTION_TYPE_MAP = {
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This lets actions.jsonl show real entity names rather than placeholders like "Agent_0"
    
    Args:
        config: Contents of simulation_config.json
        
    Returns:
        Dictionary mapping agent_id -> entity_name
    """
    agent_names = {}
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from the database and augment them with full context
    
    Args:
        db_path: Database file path
        last_rowid: Last max rowid read (use rowid instead of created_at, because different platforms have different created_at formats)
        agent_names: agent_id -> agent_name mapping
        
    Returns:
        (actions_list, new_last_rowid)
        - actions_list: List of actions; each element contains agent_id, agent_name, action_type, action_args (with context info)
    ...
    try:
        ...
        # This avoids created_at format differences (Twitter uses an integer, Reddit uses a datetime string)
        cursor.execute("""
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Trim action_args to keep only key fields (keep full content, do not truncate)
            simplified_args = {}
            ...
            # Augment with context info (post content, username, etc.)
...
def _enrich_action_context(
    ...
    """
    Augment actions with context info (post content, username, etc.)
    ...
    """
    try:
        # Like/dislike post: augment with post content and author
        ...
        # Repost: augment with original post content and author
        ...
        # Quote post: augment with original post content, author, and quoted comment
            ...
            # Get the quoted post's comment content (quote_content)
            if new_post_id:
                cursor.execute("""
        ...
        # Follow user: augment with followed user's name
        ...
        # Block user: augment with blocked user's name
        ...
        # Like/dislike comment: augment with comment content and author
        ...
        # Comment: augment with the commented post info
        ...
        # Context-augmentation failures do not affect the main flow
        print(f"Failed to augment action context: {e}")
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    - Common config: LLM_API_KEY, LLM_BASE_URL, LLM_MODEL_NAME
    - Boost config (optional): LLM_BOOST_API_KEY, LLM_BOOST_BASE_URL, LLM_BOOST_MODEL_NAME
    
    If a boost LLM is configured, parallel simulation can use different API providers for different platforms to improve concurrency.
    ...
    # Choose which LLM to use based on parameters and config
...
class PlatformSimulation:
    """Container for a single platform's simulation result"""
    def __init__(self):
        self.env = None
...
async def run_twitter_simulation(
    ...
    # Twitter uses the common LLM config
    ...
    # If an agent is missing from the config, fall back to OASIS's default name
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed rowid in the database (use rowid to avoid created_at format differences)
    ...
    for round_num in range(total_rounds):
        # Check if an exit signal was received
        ...
        # Fetch actually-executed actions from the database and record them
...
async def run_reddit_simulation(
    ...
    # Reddit uses the boost LLM config (if available, otherwise fall back to common config)
...
async def main():
    ...
    # Create a shutdown event at the start of main() so the entire program can respond to exit signals
    ...
    # Initialize log config (disable OASIS logs, clean up old files)
    ...
    # Store the simulation results of both platforms
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"Simulation loop completed! Total elapsed: {total_elapsed:.1f} seconds")
    
    # Whether to enter wait-for-commands mode
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering wait-for-commands mode - environment keeps running")
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: do not exit after the simulation completes, wait for interview commands
    When a termination signal is received, we must:
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set the event to notify the asyncio loop to exit (let it clean up resources)
            if _shutdown_event:
                _shutdown_event.set()
